  ...
  return (
    <main className='mx-8 sm:mx-20 md:max-w-[85ch] md:mx-auto md:px-4'>
      ...
      <div className='absolute bottom-0 z-50 right-1/2 translate-x-1/2'>
        <a href='#info' className='block w-8 h-8 2xl:w-12 2xl:h-12 mb-[3vh] z-50'>
          <Image src='/cdown.svg' alt='Caret Down' height={64} width={64} className='w-full h-full' />
        </a>
      </div>
      ...
      <div className='flex flex-col md:flex-row md:justify-between mx-auto h-[85vh] mt-[10vh] w-full'>
        ...
        <div className='flex flex-col mx-auto w-[90vw] md:mx-0 sm:w-[60vw] lg:w-[35vw] text-center md:text-right justify-start md:mt-[7.5%] gap-1'>
          <div className='border-2 border-neutral-600 md:border-0 rounded-lg md:rounded-none p-2 md:p-0'>
            <p className='text-2xl md:text-4xl lg:text-5xl 2xl:text-7xl font-bold text-black'>{member.name}</p>
            <p className='text-lg sm:text-xl md:text-2xl lg:text-3xl 2xl:text-4xl font-medium text-gray-800'>{member.position}</p>
          </div>

          <p className='mt-8 sm:mt-4 text-base md:text-xl lg:text-lg 2xl:text-2xl text-gray-800'>{member.about_me}</p>
      ...
      <div id='info' className='pt-24 w-full'>
        <div className='grid md:grid-cols-2'>
          ...
          <div className='mt-14 md:mt-0'>
            <p className='text-xl md:text-2xl text-right font-medium'>📚 Books I&apos;ve Reviewed</p>
        ...
        <p className='font-medium text-xl md:text-2xl mt-12 md:mt-16'>Pitches</p>
        ...
        <PitchesTable pitches={member.stock_pitch.map((stock) => ({ name: stock }))} />
      </div>
    </main>
  );
}
